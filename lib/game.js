const MAP_SIZE = 25, POSSIBLE_ITEMS = 5;
...
for (name in mapsSpec) {
    if (mapsSpec.hasOwnProperty(name)) {
        ...
        for (i = 0; i < POSSIBLE_ITEMS; i++) {
            itemPos.x = Math.floor(Math.random()*26);
            itemPos.y = Math.floor(Math.random()*26);
            if (map[itemPos.x][itemPos.y].item == '') {
                itemName = items.names[Math.floor(Math.random()*NUMBER_ITEMS)];
                map[itemPos.x][itemPos.y].item = items[itemName];
            }
        }
        maps[name] = map;
    }
    ...
    io.sockets.on('session:connection', function (socket) {
        ...
        db.users.find({login: session.user}, function (err, user) {
            ...
            function spawn() {
                ...
                socket.emit('spawn', user.login, room, oldPos);
