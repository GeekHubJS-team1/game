...
module.exports = function (app, io) {
    function findSocketByLogin(login) {
        var i, length, clients = io.sockets.clients();
        for (i = 0, l = clients.length; i < l; i++) {
            if (clients[i].session.user === login) {
                return clients[i];
            }
        }
    ...
    io.sockets.on('session:connection', function (socket) {
        ...
        db.users.findOne({login: session.user}, function (err, user) {
            ...
            socket.on('duel:proposition', function (pos, opponent) {
                var name = map[pos.x][pos.y].user;
                findSocketByLogin(name).emit('duel:proposition', pos, opponent);
            });
            socket.on('duel:finish', function (opponent) {
                ...
                db.users.findOne({login: opponent}, function (err, opponent) {
                    opponent.socket = findSocketByLogin(opponent.login);
                    ...
                    change = Math.ceil(looser.level/2);
                    winner.level += change;
                    db.users.update({_id: winner._id}, {$set: {level: winner.level}});

                    winner.socket.emit('level', winner.level);
                    opponent.socket.emit('duel:stop');
                    winner.socket.emit('duel:won', looser.login, change);
                    looser.socket.emit('duel:lost', winner.login);
                });
            });
            socket.on('duel:stop', function (user) {
                findSocketByLogin(user).emit('duel:stop');
            });
