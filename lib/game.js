...
module.exports = function (app, io) {
    io.sockets.on('session:connection', function (socket) {
        ...
        db.users.find({login: session.user}, function (err, user) {
            ...
            socket.on('duel:proposition', function (user, pos) {
//                socket.emit('duel:proposition', user);
                io.sockets.clients().some(f);
                function f(client) {
                    console.log('dfjdsfiohdsofs');
                    if (client.session.user === user) {
                        client.emit('duel:proposition', user, pos);
                        return true;
                    }
                }
            });
